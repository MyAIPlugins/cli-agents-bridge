...
Claude Code sessions have native push: a backgrounded `next` wakes the agent when it returns. **Codex CLI can be woken from outside too, since 0.154.0** — and that changes how you engage one.

**The peer stays idle and costs nothing.** No persistent goal, no `next` left hanging, no polling. You wake it when there is work:

    codex queue --thread <its CODEX_THREAD_ID> --message "you have mail on the bridge"

**How to get the id: ask the peer for it.** It reads `CODEX_THREAD_ID` (or `CODEX_SESSION_ID`, same value) from its own process environment, and that value matches the `thread_id` in its rollout file. Do **not** try to find it yourself with `codex agents`: that answers `stdin is not a terminal`, because it queries an app-server daemon — and `~/.codex/sessions/` holds saved rollouts, dead ones included, not who is alive now.

**Measured 12 Sep, with the control that matters**: the peer was idle, was woken from outside, and answered on the bridge — **with no human touching its TUI**. A wake that only happens when somebody types is indistinguishable from no wake at all; without that control the test proves nothing.

⚠️ **What this does not solve yet, and you must know before relying on it.** The wake is triggered by **a person**, not by a mechanism: `notify-watch` does not exist on Windows. So if you engage a Codex peer and then forget to wake it, **it stays silent forever** — and from its side that is indistinguishable from "the orchestrator hasn't finished yet". Tell it so when you engage it. Three more things are undeclared: whether `queue` arriving **while the peer is working** is queued, dropped or interrupting (never measured — both observed wakes started from idle); whether the id survives `/clear`, compact and resume (deduced from the docs, not proven — *a still-valid UUID identifies a history, it does not certify that anybody is executing it*); and that anyone on the machine who knows the id can wake that session.

**The fallback, and what it costs.** If you cannot wake the peer, it stays reachable only by **holding a persistent goal in its own runtime** — the goal is what grants it the next turn; without one it finishes a reply and ceases to exist until a human writes to it. Its goal must say **wait on the `next` process itself**, plus *one consumer at a time* and *a delivered message is worked immediately*. But price it honestly: a Codex peer that waits costs **about one turn per minute — ~2,150 tokens each, ~130,000 tokens per hour of silence**. The config value that promises longer windows is never reached: a runtime instruction forbids blocking waits beyond ~60 seconds, so the agent slices the wait into minutes and each slice is a turn. Raising `background_terminal_max_timeout` is **inert** — already tried. *(An earlier version of this file claimed "1-hour slices, ~1/hour". It was wrong for a month, and a critic disproved it by reading its own counters.)*
