...
Such a peer stays reachable by **holding a persistent goal in its own runtime** — the goal is what grants it the next turn; without one it finishes a reply and ceases to exist until a human writes to it. Its goal must say **wait on the process, never sleep** and *one consumer at a time*. `next` has no window: it hangs until mail arrives, so **the wait is `next` itself** and no interval is needed — the cadence comes from the runtime interrupting the wait. Two opposite failures on one day: a peer that did not wait on its background terminal re-polled every 10-30 seconds, and a peer given an explicit interval added `sleep 300` between rounds, burning a turn per nap and staying blind to a question its own `next` had already delivered. The goal must also say that **a delivered message is worked immediately** — obvious until a timing rule sits next to it, at which point the timing rule wins and the message waits. That belongs in the peer's own skill, not in your brief — but if a peer answers once and then goes quiet, **a missing goal is the first thing to check.**
