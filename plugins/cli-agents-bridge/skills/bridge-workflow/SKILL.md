...
Such a peer stays reachable by **holding a persistent goal in its own runtime** — the goal is what grants it the next turn; without one it finishes a reply and ceases to exist until a human writes to it. Its goal must spell out the polling schedule and say *one consumer at a time*; neither emerges on its own, and a peer with no interval at all re-polls every few seconds. The schedule that fits how mail actually arrives — in bursts, right after an exchange — is a backoff: **5 minutes, then 10 after three empty rounds, then 20 permanently after three more, resetting to 5 only when a message lands.** A flat interval is wrong in both directions: too slow when a brief is imminent, too frequent when nothing has happened for hours. That belongs in the peer's own skill, not in your brief — but if a peer answers once and then goes quiet, **a missing goal is the first thing to check.**
