...
- **F-40 — NUOVO, PRIMA feature v0.5 (bootstrap auto-discovery resiliente + naming adattivo)** — richiesto esplicitamente da Alan dopo un caso osservato (2026-05-31): un ESC fresco, dato solo skill `esecutore` + `cab-bridge-awareness` e nessun altro contesto, ha **sprecato turni** in esplorazione/raffica-comandi/falso-allarme-shell prima di registrarsi; e il VAL "storico" era su un data dir diverso (`teams/cab-dev`) → **mismatch di canale invisibile** (F-20). Serve un comando one-shot — es. `cab-bridge bootstrap --role=<r>` — che: (a) usa default data dir + auto-scope (zero `CAB_DATA_DIR`/`--team`/id da inserire); (b) **scopre** un peer già registrato nello scope; (c) si registra **adattando il naming** (peer `VAL-xyz` → io `ESC-xyz`, e viceversa); (d) se `esc`, entra in `listen --wait-one`. Zero config, zero artefatti da trascrivere → LL-13 applicato alla **fase più a rischio per un agente fresco** (il bootstrap) + chiude la classe mismatch-canale. **Design-first.** Insieme a F-36 è la coppia di testa di v0.5.
