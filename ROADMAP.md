...
**Fase**: **v0.2.3 SHIPPED 2026-05-30** — tag `v0.2.3` + [GitHub Release](https://github.com/MyAIPlugins/cli-agents-bridge/releases/tag/v0.2.3) con **binari prebuilt multi-OS** (GoReleaser; workflow `release.yml` primo run SUCCESS in 37s; 4 asset darwin/linux × amd64/arm64 + `checksums.txt`). Include skill pubblica `bridge-workflow` (role-agnostic, bundle plugin) + README role-agnostic + version-injection-da-tag. **v0.2.2 SHIPPED lo stesso giorno** (F-12 osservabilità + F-10 wake + F-5 team + F-9 outbox; tag + Release). Deferred: **F-17 auto-isolamento per project-root (NEXT)** — elimina il workaround manuale CAB_DATA_DIR/--team: scope auto-derivato da `.claude`/`.git` root, `peers` filtra di default per lo scope del cwd (`--all` per globale). Pain quotidiano dal feedback multi-progetto 2026-05-30, prioritario su: F-11 race cleanup, SC-3 ownership wiring, bump action a Node 24 (deprecation warning CI, non bloccante).
