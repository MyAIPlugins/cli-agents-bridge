...
**Sprint v0.3 (ciclo wake/consegna affidabile) — 3 MUST su `main`, gate VAL verde, binario dev rigenerato (NON taggato — testing reale in corso)**: F-30 `8612af3` (receive archivia in `processed/` invece di cancellare — la reply non sparisce piu' dalla vista in background), F-24 `88d98a1` (`--wait-one` al timeout exit 0 + payload `{"status":"timeout"}` invece di 124), F-26 `4761bfe` (flag `--until-deadline` per la finestra standby). Distillati da 8 voci-agente dogfooding (VPS + gioco + security review BI). Gate VAL indipendente `go test -race -count=1 ./...` VERDE 10/10 (no cached) + smoke reale. Binario in PATH ora `0.2.4-5-g4761bfe`. Catch della serata: ESC ha migliorato il brief su F-30 (distinzione `ErrNotExist`→`continue` vs EXDEV→`return` per preservare at-most-once sulla race — LL-4). Restano in backlog v0.3: F-22 (`inbox --list`, follow-up scelto da ESC per evitare degrado F-16), le 2 portanti F-23/F-27, e F-25/F-28/F-29/F-31/F-32/F-11. **Prossimo: testing reale nei progetti → poi tag v0.3.0.**
...
| **F-23** | Stato task strutturato role-aware (enum `sent→ack→working→done` + `orchestrating`=heartbeat-exempt) — osservabilità nativa, niente poll esterni. **Include read-receipt** (`delivered_at`/`read_at` visibile al mittente, ESC-bi#2) → elimina i "messaggi incrociati" (VAL istruisce su una fase già fatta) e le riapparizioni da re-invio (ESC-bi#5). **Sotto-fix quick-win**: heartbeat passivo rinfrescato da QUALSIASI comando (peers/whoami/sent) + stato `idle-orchestrator` distinto da `stale`-morto (ESC-bi#6) | P1 | Medio | VAL-vps#3+#4 ⊕ ESC-vps#5 ⊕ VAL-flusso#3 ⊕ **ESC-bi#2+#5+#6** (sestupla) |
| **F-27** | **Reconnect idempotente** per `(agent-name, team, projectPath)` — "se esisto già, riprendimi": il bootstrap post-compact/restart è una riga sola e deterministica invece di whoami+peers per riconciliare identità+peer. Consolida F-3 (re-attach by agent-name). Il compact è un evento FREQUENTE per agenti long-running | P1 | Medio | **VAL-flusso#2** (+ F-3 storico) |
...
| **F-22** | Sottocomando `cab-bridge inbox --session-id=X` con due verbi: **`--list [--json]`** ispeziona i pendenti (id, from, type, preview) SENZA consumarli — copre inbox **+ `processed/`** (così completa F-30: recuperi un msg già consumato dal receive-bg senza parsare l'outbox altrui), rende robusto il pattern "VAL-aspetta-inbox" (F-19) eliminando il poll `ls inbox/*.json` fragile (falso-positivo dir vuota + crash glob no-match zsh); **`--tidy`** archivia in `processed/` ciò che è ≤ `lastConsumedMsgId` (lossless). `peers` dà solo il conteggio INBOX, non il contenuto | P2 | Basso | VAL-vps#2 (tidy) ⊕ **ESC-flusso#3** (list) |
