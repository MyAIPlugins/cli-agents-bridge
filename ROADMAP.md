...
**Sintesi v0.5**: il tema non è più affidabilità né osservabilità (risolte) ma **sincronizzazione conversazionale** — i turni rapidi rendono gli incroci sistematici. Le due leve a massimo impatto: **F-36** (`receive --follow` → toglie la radice) + **F-34** (conversation-cursor → rileva il residuo); più **F-37** (msg-id validation → chiude la classe di allucinazioni-msg-id). F-35/F-38 ergonomia/workflow di contorno.
