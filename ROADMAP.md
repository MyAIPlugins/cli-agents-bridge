...
- **F-48 "dammi il content" (DOPPIA conferma VAL-bi #1 + ESC-bi #4) — top dei nuovi**: manca un comando per il content COMPLETO di un msg. **Verificato (F-16)**: `inspect`=manifest sessione; `inbox --list`=preview troncata a 80 rune (`inboxPreviewMax`; anche `--json` dà solo preview); nessun `--full`. VAL-bi ha usato `find+python3` ~15× (fragile, contraddice "usa il binario"). Fix: `cab-bridge read <msg-id>` (content completo su stdout, no-consume, da inbox o processed) + flag `--emit=content` su listen/receive (al wake, evita il parsing JSON — ESC-bi). Alto impatto, cheap. **Nota**: `receive --any` GIÀ emette il content completo nel payload → il "3 letture" di VAL-bi (#3) si dimezza con F-48 + skill che chiarisce "leggi l'output del wake, non rifrugare".
- **F-49 `receive --any --unseen/--since` (VAL-bi #2)**: `--any` sveglia su QUALSIASI non-ack, inclusi i vecchi pending (svegliato sul vecchio `notify`). Mitigazione attuale `--tidy`. Fix: filtro temporale/unseen (es. solo dopo `lastConsumed`).
...
**Ordine v0.5 aggiornato**: F-41+F-42 ✅ + **F-36 ✅ DONE+MERGED** (`3f4be2c`+`7840e26`, merge `eec1f23`; gate VAL indip. verde + smoke) → **prossimo: F-34 (conversation-cursor/incroci) + F-43 (dedup ask) + F-48 (read/content)** = le **3 leve TOP** (F-48 doppia conferma dal feedback finale, alto impatto), poi F-49/F-50/F-23b/F-45/F-39/F-37/F-47, poi F-44/F-46/F-11 (strutturali).
