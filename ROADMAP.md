...
- **F-51 (NUOVO — viola CLAUDE.md "no catch muti", prioritario pre-run-lunga)**: `consumeInboxEntry` (`drain.go`) + `emitInboxOnce` (`poll.go`) skippano un msg **illeggibile/malformato** con `return nil,false` SENZA log né contatore (verificato: 6 `return nil,false`, zero `Fprintf`; `scanForReply:161` invece LOGGA → **incoerenza interna**). In una run 24h non-presidiata un msg scartato è invisibile. Fix: log stderr + contatore skip per i SOLI casi-anomalia (unreadable/malformed), NON per gli skip-attesi (dir/.tmp/non-json/predicate-reject). Cheap.
...
**Ordine**: ✅ **v0.5.0 + v0.5.1 SHIPPED** (tag + Release MIT, binario `0.5.1`) — onboarding (F-40/41/42) + wake (F-36/F-49) + fluidità (F-48/F-43/F-34) + anti-allucinazione LL-13 completa (F-43/F-34/F-37). **Backlog prossimo arco**: **F-51** log+contatore skip (audit, prioritario pre-run-lunga) · **F-52** test crossing end-to-end · **F-53** test listen+receive concorrenti · **F-39** riferimenti simbolici (cappello LL-13) + cursor strutturale (oltre F-34 warning) · F-23b/F-50/F-44/F-45/F-46/F-11 + chore gofmt + misura latency >3 peer.
