...
**Windows is a supported platform now**, and the reason it took a full day is that almost nothing was wrong with the product. The gate had never run on Windows, the linter had never opened a `_windows.go` file, and the release built only darwin and linux — so the port could be verified and still reach nobody. Three product defects came out of it; the rest of the day was spent on the tools that decide whether the product is good.

### Added
- **A Windows binary in the release.** Five archives instead of four: the four `tar.gz` unchanged, plus `cab-bridge_<version>_windows_amd64.zip` carrying `cab-bridge.exe`. `windows/arm64` is **excluded explicitly** — the build matrix is cartesian, and nobody here can run that artifact. The README gained a PowerShell install path that was **executed, not written**: hash check, `Expand-Archive`, run the extracted binary.
- **The gate speaks Windows.** `go test -race -count=1` runs on `windows-latest` beside `ubuntu-latest`, and `go vet` + `staticcheck` now run **once per GOOS** (linux, windows, darwin) on ubuntu — five seconds that closed a hole as old as the repository: build constraints hide per-OS files from the analyser, so a gate pinned to one platform had never seen a single one of them.

### Fixed
- **A qualified address could not resolve on Windows** (F-135). `recipient.go` asked whether a hint was absolute by testing for a leading `/`, which is never true for `C:\…`, so every qualified address fell into the basename branch and failed — while the error listed the very project it had just refused. The way out of a shared basename was broken in the only case where it is needed, and `next` hands out exactly that form. Found by another repository's VAL pasting `fromAddressShellArg` as the skill prescribes.
- **The atomic rename was not atomic on Windows** (F-133). `os.Rename` is `MoveFileEx`, which cannot replace a file somebody is reading: the manifest write and `MoveToProcessed` failed under concurrency with `Access is denied`. Fixed with `FILE_RENAME_POSIX_SEMANTICS` plus a bounded retry for handles held by others (antivirus, editors) — the share-delete mode a ratified design had described as sufficient is a **precondition**, not the remedy. On Unix the file is one line: `return os.Rename`.
- **The cross-device branch was dead on Windows, and the comment above it said otherwise** (F-137). `syscall.EXDEV` is `536871040` there while the kernel returns `ERROR_NOT_SAME_DEVICE`; `errors.Is` was never true, so a cross-volume misconfiguration produced the generic message instead of the explicit one the comment promised. The predicate is per-OS now. One of the two call sites turns out to be unreachable on **every** platform — the temp file is created in the target's own directory — and says so, instead of inviting a search for the case that triggers it.
- **Two tests measured the machine instead of the code** (F-138, F-136), and four more had never run on Windows at all because `make` is absent from the development PC and they skipped in silence (lot 1c). A test asserting a heartbeat "fresher than 100ms" on a 30ms tick failed 5 runs out of 6 on shared runners — including twice on documentation-only commits — and now asserts **progress**, which a slow runner delays instead of breaking. Both were replaced by oracles that make the fault certain rather than likely, and both are proven by mutation: remove the fix and they fail.

### Changed
- **The GoReleaser version is pinned to `v2.18.1`** on both sides. `release.yml` carried the moving range `~> v2` while the configuration already used `formats:`, the ≥ 2.6 spelling — the same shape as `go-version: stable`, which broke the gate on 11 August with nobody touching the repository. The locally documented verification commands said `@latest`, so a maintainer following the instructions verified with a different version than the one that publishes.
- **Releases are created as drafts.** A tag now *prepares* a release instead of publishing one. This is a net, not a gate: the release workflow still does not check that the tagged commit ever passed CI, and that remains an open lot — read as "the tag problem is handled", this line would prevent somebody from building the control that is actually missing.
...
### Added
- **`internal/shellarg`** — the bridge renders any token it prints as **one POSIX shell word**, instead of asking the reader to quote it. "Always between quotes" holds for a space and breaks on an apostrophe (`Alan's Project` → `zsh: unmatched '`): no rule applied by hand covers every path, so the algorithm moved from the reader to the renderer. `peers` quotes the `SCOPE` column only where it must — the ordinary case reads as before — and keeps `--json` raw; `next` carries **`fromAddressShellArg`** beside `fromAddress`, the first to paste and the second to parse. Every remediation printed by the verbs and by `join` is rendered the same way, **including the role**: roles are not an enum (`register --role='browser tester'` exits 0, by design) and were being reprinted bare next to a quoted path.
- **An agent-name grammar** (F-124, lot 1). A name is one shell word — ASCII letters, digits, `_`, `.`, `-`; no leading `-` or `.`; never `@`, which separates name from scope — and the verbs take it as a positional. `join --agent-name` and `register` reject anything else and print **the rule**, not a mechanism: three earlier versions of that message each explained a mechanism that was false for some member of the class.
- **Reversible derivation** for names that come from a directory rather than a person: a byte outside the grammar becomes `_XX` (uppercase hex of the byte) and the marker `_` itself becomes `_5F`, so the function is injective *by construction* and the proof is the inverse. A 16-bit digest was tried first and refuted by building a collision. A one-time notice says what was derived from what, printed **after** the write and only for a genuinely new registration.
- `docs/dev-conventions.md` is now the **published source** for the two rules that code comments cite (no implicit fallbacks, zero runtime dependencies).

### Changed
- **`register --resume` resumes by identity, not by name.** The name no longer takes part in the lookup, so a changed derivation cannot strand an existing session. Two live sessions on the same path with different names — the supported `--force-new` shape — are an **ambiguity error listing both**, not a tie broken by the newest heartbeat (which resumed the empty one and left the mail in the other). The repair command it prints carries `--project-path`: `join` without it uses the cwd, and executed to the letter from another directory it created a new session instead of repairing.
- **`register` and `join` read the outcome from the result, not from the clock.** Both decided "was this a resume?" by comparing `StartedAt` with the time taken just before the call; a manifest from another life — clock rollback, a restored VM — made `join` report `registered-new` for a reclaimed session.
- **Lock errors state facts and no remedy** (F-126). `session lock held by live process: lockPath=… holder pid=…` used to append `(use --force-new to override)`: ten of the lock's eleven callers pass `forceNew=false` hard-wired, so the flag was not merely absent from their commands — it was inexpressible, and wrong for all ten (they hold the lock for milliseconds; retrying is right). The primitive now reports `lockPath` and `holder pid` and nothing else; the one path where a deliberate second instance is the answer says so itself.
- **README rewritten around the binary.** The plugin installs three files and no executable, and the previous text sent readers to `listen` and `ask --to`, both gone in 0.8. Now: install first (download, `mkdir -p ~/.local/bin` — `install` does not create its destination — `install -m 755`, then `cab-bridge version` as the check that must pass before anything else), the plugin second, the loop third. Validated black-box by two independent fresh readers, in separate sandboxes and not told what was being measured, both reaching a round-trip from the repository address alone. The checksum line says what it proves: integrity of the download, not origin.
- **`CLAUDE.md` left the repository** (F-127). Claude Code loads it as *project instructions* in any clone, so a contributor's agent was receiving the maintainers' internal workflow as orders. The defect was auto-execution, not content; the file stays on the maintainers' disks, and the four Go comments that cited it now point at `docs/dev-conventions.md`.

### Fixed
- **A name with a space was delivered as a fragment to the wrong agent, exit 0** (F-124): `tell ESC bridge`, pasted from the `AGENT_NAME` column, sent six bytes of the name to `ESC`. Closed by the grammar above. The other three shapes already failed closed but diagnosed the wrong cause ("quote the message as one argument" when the message *was* quoted).
- `join`'s collision guardrail and the derivation notice: the first quoted the path and left the role bare; the second ran before `Manager.Register` and announced, on `--resume`, a derivation the command then did not perform.
- **Makefile.** `install-dev` linked `$(PWD)` — the caller's cwd, not make's — so `make -C <repo>` from elsewhere installed a **dangling symlink with exit 0**; it now links what it just built and every path is quoted, so a checkout under a directory with a space builds and installs. `lint` finds `staticcheck` where `go install` puts it, pins the version in `.staticcheck-version` (the one file the Makefile and CI both read), verifies the binary that *runs* matches the pin, and fails instead of narrating. The make test suite no longer depends on `.git` and sanitises the three `GIT_*` variables (`GIT_DIR`, `GIT_INDEX_FILE`, `GIT_WORK_TREE`) that could redirect it.
- **CI** runs `go test -race -count=1` (it ran without `-count=1` under `cache: true`, so a change to a shared helper did not re-run the packages using it) and the pinned `staticcheck`; the job named "lint + race" only ran `vet`, and now does what it says.
- Five `staticcheck` warnings, one of which was a missing assertion in a test.

### Known limitations
- **F-125 — derived names were never unique across projects of one scope**: `alpha/work` and `beta/work` both derive `work`, and `tell work` then exits 1 naming two live agents. Pre-existing and measured identical across three revisions, not introduced here; `--agent-name` is the way out until it is closed.
- **F-119** — one integration test with real subprocesses times out under machine saturation (2 of 11 runs during a four-agent peak, 0 of 24 otherwise). Measured, not attributed.
