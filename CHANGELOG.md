...
- **The rule the grammar rests on**: *every token `peers` prints can be pasted into a command.* Break it and the discovery list is wider than reachability again — the very defect this closes, reappearing on the branch meant to fix it.
...
- **Provenance**: `fromScope` is **additive** — `fromProject` keeps its value and meaning — shown in `next` only when it differs from the reader's, alongside a `fromAddress` already assembled so an agent copies instead of building. On legacy messages it is absent and never inferred.
