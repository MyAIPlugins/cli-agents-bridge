...
- **SC-3 ownership check (active since v0.8.0)**: `internal/security/perms.go::ReadOwnedFile` opens with `O_NOFOLLOW|O_NONBLOCK`, refuses anything that is not a regular file, `fstat`-s **the descriptor it will read from**, and reads from that same descriptor — so there is no window between the check and the use, and a symlink is refused at `open` rather than resolved and then inspected (an `Lstat`-then-`open` would have been the same TOCTOU under another name).
  - **Covered**: manifests, messages in `inbox/`/`outbox/`/`processed/`, session state (`wake-cursor.json`, `listener.json`, `reply-txn.json`, the lock file, `notify-watch/<name>.json`) and `config.json` — everything inside our data dir. The Patil data dir, read only by `migrate-from-patil`, is out: a perimeter we did not create, and refusing it would mean a migration that cannot read its own source.
  - **NOT covered, stated rather than half-claimed**: the directories *above* the leaf are not revalidated on every read. Doing that properly means walking each component with `openat(O_NOFOLLOW)` on every single read, and the residue it buys is narrow — SC-7 keeps the data dir 0700 and owned by us, so from the first run onward the tree cannot be altered by anyone else. What stays uncovered is an intermediate directory swapped for a symlink *during* the loose-perms window that SC-7 repairs.
