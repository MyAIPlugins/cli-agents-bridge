...
- **SC-3 ownership check (active since v0.8.0)**: `internal/security/perms.go::ReadOwnedFile` opens with `O_NOFOLLOW|O_NONBLOCK`, refuses anything that is not a regular file, `fstat`-s **the descriptor it will read from**, and reads from that same descriptor — so there is no window between the check and the use, and a symlink is refused at `open` rather than resolved and then inspected (an `Lstat`-then-`open` would have been the same TOCTOU under another name).
  ...
  - **NOT covered, and the gap is NOT narrow — open defect at the time of writing.** The directories *above* the leaf are not validated. An earlier revision of this entry called that residue narrow; an independent reviewer reproduced it and it is not. A single intermediate symlink defeats the whole control: with `<data-dir>/sessions` pointing elsewhere, `os.ReadDir` follows it, every file behind it is a regular file legitimately owned by us, so **every leaf check passes and SC-3 has nothing to object to** — an empty data dir was made to enumerate another one's live sessions, with SC-7 satisfied throughout. The same shape applied to `archive/` sends `purgeOldArchives` to `RemoveAll` **outside** the data dir, which reopens TM-4 and TM-5. Until the parent components are validated, SC-3 should be read as *"the leaf is what it claims to be"*, and **not** as *"the loose-perms window is remediated"* — that second claim is what this document got wrong twice.
