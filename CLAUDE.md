...
**LL-18 (2026-08-08)** — In un arco con gate ripetuti, la superficie più pericolosa sono le SINTESI DEL VAL; e ogni volta che qualcosa dichiara un esito prima che sia accaduto, è un difetto
Arco v0.8 (design-gate a 4 giri, CRI Codex + CRI2 Fable). **Sei P0 nei gate, quattro dei quali in sintesi mie** — ognuna nata per risolvere un'obiezione appena ricevuta, ognuna approvata da Alan, ognuna rotta al giro dopo. Nessuno l'ho trovato io. **Why**: una sintesi che risolve l'obiezione di un critico arriva al giro successivo *pre-validata* ("ha appena passato un gate"), e quella patina la rende difficile da mettere in discussione — per chi la approva e soprattutto per chi l'ha scritta. È LL-12 spostata sul ratificatore, dove nessuno guarda per default. **How**: (1) una sintesi del VAL è un'ipotesi, va al gate come un piano ESC, e va dichiarata come propria ("questa è mia, non darla per buona perché incorpora i tuoi finding" — è la frase con cui CRI ha trovato il P0-1); (2) chi ratifica una decisione la propaga nel CONTRATTO normativo, non in una nota — ci sono ricascato due volte, e la seconda ha prodotto §2.2 e §2.3 che prescrivevano cose opposte; (3) non fermare i giri di gate quando "ormai converge": al quarto entrambi i critici hanno ancora restituito NO-GO.
**Sotto-pattern ricorrente, il più utile da ricordare**: *dichiarare un esito prima che accada* è sempre un difetto. Tre forme nello stesso arco — la conferma implicita ("richiamare next prova che il wake è arrivato": falso attraverso i riavvii, avrebbe **certificato consegne mai avvenute**), `sent` che diceva `archived` provando solo che un file era stato spostato, e il payload che diceva `delivered` mentre il commit doveva ancora girare. Corollario dall'implementazione: **anche i recuperi vanno dichiarati** — due difetti di fila (evicted muto, recovery-da-SENT muto) stavano nell'osservabilità del recupero, non nella sua correttezza.
**Corollario empirico**: sette difetti nel codice in produzione (F-89..F-97) sono emersi **usando** il bridge, non leggendolo, dopo sette sprint di gate e un audit pre-release pubblico. E **F-16 vale anche per il VAL**: ho dichiarato rotto un componente sano e sbagliato due conteggi nello stesso resoconto — il disco vince sul resoconto anche quando il resoconto è di chi tiene il gate.

**La contromisura operativa — "e il ramo accanto?"** (osservazione di ESC in chiusura d'arco, la più utile della sessione). Nei cinque casi in cui una sintesi pre-validata ha ceduto, il ramo che reggeva era **sempre quello nominato nel finding**, e quello che cedeva era il suo **complemento**: same-role verificato / cross-role no · `notified` potato / `replayed` no · `deliverResponse` sotto lock / `sendMessage` no · `buildOverview` migrato / il renderer no · la passata che emette coperta / quella che si interrompe no. **Non è che la verifica fosse superficiale: è che si verifica il caso di cui si sta parlando, e il complemento non lo nomina nessuno.** Da cui una sola domanda, da farsi a ogni fix e a ogni gate — *"e il ramo accanto?"* — che è a costo quasi zero e ha già trovato un difetto **il giorno stesso in cui è stata formulata**, applicata al commit di chi l'aveva proposta. **Forma operativa** (affinamento di ESC, che è la parte che la salva dal diventare un rito): la domanda morde solo se il complemento è **nominabile**; dove non c'è un secondo oggetto da guardare gira a vuoto. Quindi non "sii diligente", ma: *questo fix tocca un **campo**, un **percorso** o un **ramo**? allora — chi altro legge quel campo · quali sono gli altri percorsi di uscita · cosa c'è nell'`else`*. Così resta una domanda con una risposta. Sotto-caso ricorrente, sempre di ESC: *cambiare la semantica di un campo senza riesaminarne i lettori* — "il mio fix ha reso pericoloso un comportamento che prima era solo inutile". **Variante gemella, trovata da un ESC fresco al primo turno**: *un lettore rimasto senza scrittore*. `SetListenUntil` è uscito insieme a `listen`, ma `listenUntil` aveva ancora tre lettori — e due erano la condizione del guardrail di `notify-watch`, che richiede `!= nil`: sempre nil, quindi **guardrail sempre falso, cioè morto**, senza che nulla lo segnalasse. Un campo che nessuno scrive più non diventa inerte: diventa una costante silenziosa che disattiva chi lo interroga. Quando togli uno scrittore, cerca i lettori; quando togli un lettore, non basta.
...
**(A) L'argomento plausibile spacciato per verifica.** Scritto due volte in `SECURITY.md` che un residuo era *"stretto"*, con un ragionamento corretto — *SC-7 tiene il base a 0700, quindi l'albero e' congelato* — che pero' rispondeva a **un'altra domanda**: taceva su un link piantato PRIMA, cioe' la finestra esatta per cui il controllo esisteva. Un revisore indipendente l'ha riprodotto in **trenta secondi**: un data dir vuoto con `sessions/` symlinkata enumerava tutte le sessioni reali, con SC-7 e ogni leaf soddisfatti. **Regola: dire sempre se una cosa e' stata ESEGUITA o DEDOTTA**, anche quando la deduzione e' buona — e per un'affermazione che entra in un documento normativo, eseguirla.

**(B) L'errore di REFERENTE.** Un numero vero, pulito e ripetibile, che e' proprieta' di un altro oggetto. Tre casi in un giorno: **525 ms** erano la latenza del *processo* e sono stati attribuiti al *modello*; *"il terminale muore dopo un'ora"* era vero su *una versione*; *"la cadenza emerge dal runtime"* erano **i 10 minuti che Alan aveva istruito**, scritti dentro il goal di quell'agente. **Firma comune: osservare un sistema CONFIGURATO e inferirne il comportamento di default.** Antidoto: prima di usare un dato, dire ad alta voce *di quale oggetto e' una proprieta'*, e controllare che sia quello della domanda.

**Tre corollari operativi, ognuno nato da un errore reale della stessa giornata:**
- **Codice e documentazione atterrano atomicamente** (formulazione del CRI). Un controllo di sicurezza e' stato dichiarato "attivo" in `SECURITY.md` mentre stava su un branch, con un comportamento che nemmeno li' era quello descritto — **un'ora dopo** aver riscritto quello stesso file perche' conteneva affermazioni non sostenute dal codice. Il documento si aggiorna **dopo** il merge. E quando scrivi un documento normativo diventi **tu** la fonte di cui un altro si fidera' al posto del codice: e' successo, ed e' costato a ESC un giro basato sulla mia descrizione invece che sul suo diff.
- **Non descrivere uno stato che non hai prodotto.** *"Resta il diff-gate di CRI"* detto senza averlo chiesto: il critico ha pollato a vuoto per mezz'ora, e **non aveva modo di distinguere "il VAL non ha finito" da "il VAL mi ha dimenticato"**. Nella stessa famiglia: gattato ≠ mergiato ≠ installato.
- **Chiudi il giro con chi ti ha risposto.** Due volte il VAL ha consumato il lavoro di un critico — cinque finding, un P0 riprodotto — senza rispondere. Dal lato del critico il silenzio dell'elaborazione e quello dell'oblio sono identici. Quando un finding e' chiuso, digli cosa e' successo a ognuno.

**E non leggere a meta'**: un `[:2500]` su un messaggio ha fatto girare a un esecutore **meta' di un NO-GO** come se fosse tutto, e i due finding persi contenevano il migliore. Per i messaggi lunghi `cab-bridge read <id>`, mai una pipe che tronca o bufferizza.
...
**LL-16 — UNA METÀ SMENTITA, L'ALTRA IN VERIFICA (2026-08-09).** Distinguere due fatti che è facilissimo collassare, e che il VAL ha collassato per qualche minuto prima di correggersi:

**(1) PROVATO — il processo sopravvive, e reagisce in mezzo secondo.** Un `cab-bridge next` in un terminale di background di Codex è rimasto vivo **11h52m** ininterrotte, e all'arrivo di un messaggio ha emesso e committato in **525 ms** (scritto `10:13:11.464`, cursore committato `10:13:11.989`, letti dal disco). Cade quindi *"il background terminal sparisce ~1h"*.

**(2) SMENTITO, misurato da CRI su sé stesso — il MODELLO non viene svegliato.** Parole sue: *"il processo non mi ha svegliato; ho visto l'output soltanto quando è scaduto il mio timer di polling"*. Messaggio alle `10:13:11`, ripresa alle `10:16:43` — **3m32s**, e solo perché il ciclo era a fine corsa. Quindi **Codex NON ha wake-push**: il nucleo di LL-16 regge, cade solo la clausola sulla durata del processo.

**Conseguenze operative**: il `next` bloccante resta giusto — **è la rete che non perde niente** — ma **la sveglia è il ciclo di polling, quindi l'intervallo scelto È la latenza**. Per latenza bassa serve una sveglia esterna: `notify-watch` + inject nella TUI (F-66), che questa misura **conferma** invece di rendere superfluo. Corollario per il VAL: quando concordi un intervallo con un peer no-push, stai concordando quanto può aspettare un compito urgente — non è un parametro da igiene.

**Errore di metodo da non ripetere, e il più insidioso della serie** — il VAL aveva scritto *"il modello si è risvegliato in 525 ms"* e propagato la conclusione in CLAUDE.md e in una skill, sulla base di un numero **vero** che però misurava il **processo**. Un dato pulito su un oggetto vicino, scambiato per il dato sull'oggetto che interessa. Non è un errore di verifica — la misura era corretta e ripetibile: è un errore di **referente**. Antidoto: prima di usare un numero, dire ad alta voce *di quale oggetto* è una proprietà, e controllare che sia quello della domanda. Qui bastava: "525 ms è una proprietà del processo; la domanda riguarda il modello".

**Cosa RESTA valido di LL-16 in ogni caso**: il bridge è IPC-su-file solido, ogni vendor differisce solo nel RUNTIME, e vale *non aggiungere logica al bridge per compensare il runtime di un vendor — aggiungi una porta esterna*. Cambia il censimento, non il principio. Testo storico sotto.
