...
4. **Fix BUG-4** `cab cleanup` default scope=my-session, --scope=global opt-in con confirm prompt. **Sprint 3 finding**: ErrConfirmRequired exit 3 distinct in non-tty + pre-delete archive `archive/<date>/<id>/` + retention sweep GDPR-1 wired. **Inbox policy A→B migration completata Sprint 3** (refactor `poll.go` da `os.Remove` a `MoveToProcessed` con RFC3339 timestamp prefix per audit trail e foundation transcript v0.3).
...
| `routing` | Role-based compat check pre-send (`ValidateSendPair`) — hub-and-spoke val-centric default + `--allow-mesh` override esplicito + **observer-cannot-send STRUTTURALE** (no flag override possibile, constraint architetturale vs default convenience). **Sprint 3**: 60 LOC + 15 sub-test green. | BUG-3 |
| `cleanup` | Scope-aware: my-session default, --scope=global con TTY confirm + ErrConfirmRequired exit 3 non-tty, pre-delete archive `archive/<date>/<id>/`, retention sweep GDPR-1 (default 7gg). Usa `cfg.StaleSeconds` unica fonte di verità (no hardcoded). **Sprint 3**: ~90 LOC + 6 sub-test green. | BUG-4, BUG-8 |
